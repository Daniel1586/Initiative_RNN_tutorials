...
# A LSTM and character-based presentation for NLP sequence tagging used for multi-task learning.
    ...
    def buildModel(self):
        ...
        if self.params['charEmbeddings'] not in [None, "None", "none", False, "False", "false"]:
            ...
            if self.params['charEmbeddings'].lower() == 'lstm':
                ...
                chars = TimeDistributed(LSTM(charLSTMSize, return_sequences=False))(chars)
        ...
        for size in self.params['LSTM-Size']:      
            if isinstance(self.params['dropout'], (list, tuple)):  
                shared_layer = LSTM(size, return_sequences=True, dropout=self.params['dropout'][0],
                                    recurrent_dropout=self.params['dropout'][1])(shared_layer)
            else:
                """ Naive dropout """
        ...
        for modelName in self.modelNames:
            ...
            for classifier in modelClassifier:
                ...
                if classifier == 'Softmax':
                    ...
                    size = classifier[1]
                    if isinstance(self.params['dropout'], (list, tuple)): 
                        output = LSTM(size, return_sequences=True, dropout=self.params['dropout'][0],
                                      recurrent_dropout=self.params['dropout'][1])(output)
                    else:
                        """ Naive dropout """ 
                        output = LSTM(size, return_sequences=True)(output)
                        if self.params['dropout'] > 0.0:
